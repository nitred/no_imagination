...
class GAN_CNN(BaseOps):
    ...
    def __build_discriminator(self, inputs, input_dim, output_dim, keep_prob=None):
        """."""
        logger.info("Entry")
        fc1 = self.fc(inputs, output_dim=1024, activation=self.leaky_relu, scope='fc1')
        fc2 = self.fc(fc1, output_dim=512, activation=self.leaky_relu, scope='fc2')
        fc3 = self.fc(fc2, output_dim=256, activation=self.leaky_relu, scope='fc3')
        fc4 = self.fc(fc3, output_dim=output_dim, activation=tf.nn.sigmoid, scope='fc4')
        return fc4
    ...
    def __build_model(self):
        ...
        with self.g.as_default():
            ...
            with tf.variable_scope("D") as scope:
                ...
                self.D1 = self.__build_discriminator(self.D1_input,
                ...
                self.D2 = self.__build_discriminator(self.D2_input,
                                                     input_dim=self.D_input_dim,
                                                     output_dim=self.y_dim,
                                                     keep_prob=self.keep_prob)
