...
Source: https://github.com/ryankiros/skip-thoughts

The file has been edited from its original version.
"""
import os
...
path_to_models = get_project_directory('skipthoughts', 'models')
path_to_tables = get_project_directory('skipthoughts', 'models')
...
def nn(model, text, vectors, query, k=5):
    ...
    Args:
        text (list of str): list of sentences
        vectors (vectors): the corresponding representations for text
        query (str): a string to search
    """
...
def load_params(path, params):
    ...
    for kk, vv in params.items():
        if kk not in pp:
            print('WARNING!!!: %s is not in the archive' % kk)
            continue
        params[kk] = pp[kk]
    return params
...
def build_encoder_bi(tparams, options):
    """Bbuild bidirectional encoder, given pre-computed word embeddings."""
    # word embedding (source)
    embedding = tensor.tensor3('embedding', dtype='float32')
